...
# 20170707
- Lowered MaxWindows to prevent infinite loop
- Incorporated patches since 20121111 from MQ2AutoLogin
###########################################################################################################################
...
#define MAX_WINDOWS 140 // had to lower this for CotF patch it never reaches 200...
...
ISXPreSetup("ISXEQAutologin",ISXEQAutologin);
...
unsigned int ISXEQAutologinXML=0;
...
int Cmd_SwitchServer(int argc,char *argv[]);
int Cmd_SwitchCharacter(int argc,char *argv[]);
int Cmd_Relog(int argc,char *argv[]);
...
bool ISXEQAutologin::Initialize(ISInterface *p_ISInterface)
{   
	...
	WriteChatf("ISXEQAutologin Loaded");
	return true;
}

void ISXEQAutologin::Shutdown()
{
    pISInterface->UnloadSet(ISXEQAutologinXML);
...
void ISXEQAutologin::ConnectServices()
{
...
void ISXEQAutologin::RegisterCommands()
{
	pISInterface->AddCommand("switchserver", Cmd_SwitchServer);
	pISInterface->AddCommand("switchcharacter", Cmd_SwitchCharacter);
	pISInterface->AddCommand("relog", Cmd_Relog);
}

void ISXEQAutologin::DisconnectServices()
{
    if (hPulseService)
...
void ISXEQAutologin::UnRegisterCommands()
{
	pISInterface->RemoveCommand("switchserver");
	pISInterface->RemoveCommand("switchcharacter");
	pISInterface->RemoveCommand("relog");
}

void ISXEQAutologin::LoadSettings()
{
    bool bChanged=false;

    ISXEQAutologinXML=pISInterface->OpenSettings(XMLFileName);
    unsigned int GeneralGUID;
    if (GeneralGUID=pISInterface->FindSet(ISXEQAutologinXML,"General"))
    {
        if (!pISInterface->GetSetting(GeneralGUID,"KickActiveCharacter",bKickActiveChar))
		{
			pISInterface->SetSetting(GeneralGUID,"KickActiveCharacter",bKickActiveChar);
			bChanged=true;
		}

		if (!pISInterface->GetSetting(GeneralGUID,"UseStationNamesInsteadOfSessions",bUseStationNamesInsteadOfSessions))
		{
			pISInterface->SetSetting(GeneralGUID,"UseStationNamesInsteadOfSessions",bUseStationNamesInsteadOfSessions);
			bChanged=true;
		}
    }
	else
	{
		GeneralGUID=pISInterface->CreateSet(ISXEQAutologinXML,"General");
		pISInterface->SetSetting(GeneralGUID,"KickActiveCharacter",bKickActiveChar);
		pISInterface->SetSetting(GeneralGUID,"UseStationNamesInsteadOfSessions",bUseStationNamesInsteadOfSessions);
		bChanged=true;
	}
    ...
    if (bChanged)
        pISInterface->ExportSet(ISXEQAutologinXML,XMLFileName);
}
		...
		if(GetGameState() == GAMESTATE_PRECHARSELECT)
		{
			...
			if(!bInGame && !threadID)
			{
				...
				if(!bUseStationNamesInsteadOfSessions)
				{
					...
					if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQAutologinXML,"General"))
						if (unsigned int sessionGUID=pISInterface->FindSet(GeneralGUID,szSession))
						{
							pISInterface->GetSetting(sessionGUID,"StationName",szStationName,64);
							pISInterface->GetSetting(sessionGUID,"Password",szPassword,64);
							pISInterface->GetSetting(sessionGUID,"Server",szServerName,32);
							pISInterface->GetSetting(sessionGUID,"Character",szCharacterName,64);
						}
				}
...
int Cmd_SwitchServer(int argc,char *argv[])
{
	...
	pISInterface->GetArgs(1,2,argv,szServer,MAX_STRING);
	pISInterface->GetArgs(2,3,argv,szCharacter,MAX_STRING);
...
int Cmd_SwitchCharacter(int argc,char *argv[])
{
	...
	if(argc>1)
	{
		pISInterface->GetArgs(1,2,argv,szArg1,MAX_STRING);
	...
	else if(WindowActive("connect"))
	{
		...
		if(pWnd = WindowMap["connect"]->_GetChildItem("LOGIN_UsernameEdit"))
		{
			if (bUseMQ2Login) {
				...
				if(szStationName[0])
				{
					if (unsigned int stationGUID=pISInterface->FindSet(ISXEQAutologinXML,szStationName))
					{
						pISInterface->GetSetting(stationGUID,"Password",szPassword,64);
						pISInterface->GetSetting(stationGUID,"Server",szServerName,32);
						pISInterface->GetSetting(stationGUID,"Character",szCharacterName,64);
					}
				}
				else
				{
